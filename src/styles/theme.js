...
export const getStyles = (theme = {}) => ({
  ...
  navBtn: (active) => ({ background:"transparent", border:"none", borderBottom:active?"3px solid #B91C1C":"3px solid transparent", color:active?"#F0EDE5":"#505050", padding:"13px 26px", fontSize:13, letterSpacing:3, fontWeight:700, cursor:"pointer", fontFamily:"'Barlow Condensed','Arial Narrow',Arial,sans-serif", transition:"all 0.15s" }),
  main:   { display:"flex", height:"calc(100vh - 102px)" },
  sidebar:{ width:195, borderRight:"1px solid #111", overflowY:"auto", flexShrink:0, background:"#090909" },
  content:{ flex:1, overflowY:"auto", padding:24 },
  charRow:(active, color) => ({ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", cursor:"pointer", borderLeft:active?`3px solid ${color}`:"3px solid transparent", background:active?"#111":"transparent", transition:"all 0.1s" }),
  ...
  buildTabBtn: (active, col) => ({ background:active?`${col}1A`:"#111", border:`2px solid ${active?col:"#1E1E1E"}`, color:active?col:"#505050", padding:"9px 20px", fontSize:13, letterSpacing:2, fontWeight:700, cursor:"pointer", fontFamily:"'Barlow Condensed','Arial Narrow',Arial,sans-serif", clipPath:"polygon(0 0,100% 0,94% 100%,0 100%)", transition:"all 0.15s", marginRight:6 }),
  statRow:{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"5px 0", borderBottom:"1px solid #111" },
});
